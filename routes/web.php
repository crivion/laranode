...
Route::get('/backups/statuses', [BackupController::class, 'statuses'])->middleware(['auth'])->name('backups.statuses');
Route::get('/backups/{backup}/download', [BackupController::class, 'download'])->middleware(['auth'])->name('backups.download');
Route::post('/backups/{backup}/restore', [BackupController::class, 'restore'])->middleware(['auth'])->name('backups.restore');
Route::delete('/backups/{backup}', [BackupController::class, 'destroy'])->middleware(['auth'])->name('backups.destroy');

// Backup schedules [Admin | User]
Route::resource('/backup-schedules', BackupScheduleController::class)->middleware(['auth'])->except(['create', 'edit', 'show']);

// Backup destinations [Admin]
Route::resource('/admin/backup-destinations', BackupDestinationController::class)->middleware(['auth', AdminMiddleware::class])->except(['create', 'edit', 'show']);
Route::post('/admin/backup-destinations/{backup_destination}/test', [BackupDestinationController::class, 'test'])->middleware(['auth', AdminMiddleware::class])->name('backup-destinations.test');
